...
async function getJSON(path,fallback){const params=new URLSearchParams(location.search);const draft=params.get('draft')!=='0';const key=path.includes('layout')?'gallery-layout-draft':path.includes('data')?'gallery-data-draft':'';const galleryId=params.get('gallery')||localStorage.getItem('active-gallery-id')||'repo';if(galleryId&&galleryId!=='repo'){try{const gs=JSON.parse(localStorage.getItem('gallery-projects')||'[]');const g=gs.find(x=>x.id===galleryId);const val=path.includes('layout')?g?.layout:path.includes('data')?g?.data:null;if(val){console.info('Using active gallery '+galleryId+' for '+path);return val}}catch(e){console.warn(e)}}if(draft&&key){try{const d=localStorage.getItem(key);if(d){console.info('Using local draft for '+path);return JSON.parse(d)}}catch(e){console.warn(e)}}try{const r=await fetch(path,{cache:'no-cache'});if(!r.ok)throw Error(path);return await r.json()}catch(e){showError(`Could not load ${path}. Using fallback data.`);return fallback}}
